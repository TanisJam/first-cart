...
export default function CardActions({ id, price, quantity, setQuantity, isCart }: CardActionsProps) {
  ...
  return (
    <CardActionsMui sx={sxActionsContainer}  >
      <Box sx={sxActionsBox}>
        <QuantitySelector title={isCart ? "" : 'QUANTITY'} {...{ quantity, setQuantity }} />
        <TotalPrice title={isCart ? "" : 'TOTAL'} {...{ totalPrice, discount }} labelTop={isCart} />
      </Box>
      {!isCart && (
